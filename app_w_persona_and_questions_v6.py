...
def generate_questions(persona):
    ...
    ペルソナの初回質問、AIチャットボット回答、ペルソナの追加質問、AIチャットボットの追加質問に対する回答

    目標: {persona['goals']}
    課題: {persona['challenges']}

    【重要】AIチャットボットがペルソナの質問に回答する場合、以下のDX推進の課題チェックリストの問いに対するベストプラクティスの回答を作成してください。
    【DX推進の課題チェックリスト】
    ...
    出力フォーマット:
       ...
       - チャットボットからの回答
       - 回答に対するペルソナの追加の深掘り質問
       - 追加の深掘り質問に対するチャットボットの回答
     
    ペルソナの質問とAIチャットボットのペルソナに対する回答の深掘り質問を考えてください。
    """
