...
def generate_questions(persona):
    ...
    以下の目標と課題に基づいて、AIチャットボットに投げかけるべき質問を考えてください。
    さらに、質問に対するAIチャットボットの回答を受けて、追加で深掘りする質問を考えてください。
    ペルソナの初回質問、AIチャットボット回答、ペルソナの追加質問、AIチャットボットの追加質問に対する回答を1セットで実施します。
    これを5セット行ったら完了です。
    ...
    「この要素を考慮することが重要です」とペルソナに伝え、DX推進の課題チェックリストの内容に基づいたベストプラクティスを回答として付け加える。
    ...
    1. AIチャットボットの導入にあたって、どのようなKPIを設定するのが望ましいですか？
       ...
       - ペルソナの追加質問: これらのKPIを設定する際の具体的な数値目標の決め方は？
       - AIチャットボットの追加回答: 数値目標は、過去のデータと業界のベンチマークをもとに設定するのが良いでしょう。
       - もしDX推進の課題チェックリストの要素が不足している場合:
         - AIチャットボット: 「KPIを設定する際に、業務改善目標とその進捗モニタリングを考慮することが重要です。具体的には、定期的なKPIレビューと成果の可視化を行い、現場との適切なコミュニケーションを維持するのが理想的です。」

    質問とAIチャットボットの回答に対する深掘り質問を考えてください。
    """
